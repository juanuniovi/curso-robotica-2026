...
> *Versión reducida para el curso: se han eliminado los requisitos marcados `[OPCIONAL]`
> en el pliego original, las cargas de pago opcionales y varios requisitos de detalle o
> redundantes; los de gestión y pruebas (`RDOC`, `RPVyA`) se han dejado en lo esencial.
> **Los identificadores omitidos se conservan como huecos** para mantener la trazabilidad
> con el pliego original.*
...
| **RGEN-02** | **Alta modularidad:** una **barcaza (bastidor) principal** común a toda la gama de configuraciones, con módulos funcionales adaptables e **intercambiables**; la arquitectura permitirá incorporar nuevos módulos, cargas de pago e implementos, y vincular otros UxV para operación conjunta. |
...
| **RGEN-20** | Sistema de iluminación con **cobertura completa 360°**, configurable por el operador, con **rejillas de protección de faros** capaces de retener piedras de ≥ 3 cm sin afectar a la emisión luminosa. |
...
| **RGEN-24** | Puntos de amarre y estiba para **transporte como carga por vía aérea** (aeronaves de ala fija) **y terrestre**, dimensionados para los factores de carga (normas de referencia aplicables). |
| **RGEN-26** | **Cabestrante** de gran capacidad y/o tracción con eslingas, y **puntos de anclaje** para rescate y autorrescate por tracción, con procedimientos de seguridad para terrenos irregulares. |
| **RGEN-27** | El UGV se basará en un **tren de tracción de ruedas** para operaciones de alta movilidad. |
| **RGEN-28** | El UGV tendrá **Puntos de Interfaz** accesibles (mecánicos, eléctricos, lógicos o combinados) para integrar las cargas de pago definidas, **sin comprometer la estabilidad** del UGV (con o sin carga montada). |
...
| ID | Requisito (resumen) |
|---|---|
| **RLT1-02** | Tren de tracción de **ruedas** en configuración **4×4, 6×6 u 8×8**, con elevadas capacidades todoterreno de dificultad promedio y mayores velocidades en terrenos llanos y consolidados. |
...
#### 3.2.2 Puntos de Interfaz (cargas de pago obligatorias)

- **RLT1-12 — Zona frontal (Punto de Interfaz 1):** interfaz electromecánico para
  **limpieza de rutas** con hoja quitanieves **(CP-01)** — anchura ≥ 2.250 mm, acero
  Brinell ≥ 450, orientable.
- **RLT1-13 — Zona superior:**
  - **Punto de Interfaz 2:** **módulo de efectores especiales (CP-04)** *(no detallado en
    esta versión docente)*.
  - **Punto de Interfaz 3 — bahía de carga** de gran capacidad: plataforma de **recarga
    simultánea de dos UAV (CP-09)**; subsistema con ≥ 2 puntos de **recarga inalámbrica
    para UGV transportables** con acceso de entrada/salida sin acción humana — función
    **Nodriza (CP-10)**.
- **RLT1-14 — Zona trasera (Punto de Interfaz 4):** enganche mecánico de remolques de
  hasta 3.000 kg **(CP-14)**.

*(El pliego original define además cargas de pago opcionales `CP-02, CP-03, CP-05–CP-08,
CP-11–CP-13`, omitidas en esta versión.)*
...
| ID | Requisito (resumen) |
|---|---|
| **RPdO-01** | Puesto de Operación **móvil**, transportable en maletas/baúles rugerizados y embarcable en vehículo tipo *shelter*; permite teleoperación directa y funciones de navegación autónoma. |
...
| **RPdO-04** | Interfaces con información suficiente para controlar el sistema en cualquier entorno y condición ambiental **sin generar confusión ni estrés** en el operador (información de conducción visual/auditiva/háptica, de misión y de estado), permitiendo **seleccionar y visualizar** las imágenes que el operador estime necesarias. |
...
| ID | Requisito (resumen) |
|---|---|
| **RCOM-01** | **Tres módulos de comunicaciones alternativos:** (a) módulo **5G** para misiones de seguridad civil; (b) módulo de **radio** con protección de transmisión, interoperable con sistemas en dotación y robusto ante perturbaciones; (c) **terminal satélite comercial** sustituible por uno de mayores prestaciones. |
...
| **RCOM-03** | Modo de aseguramiento de las comunicaciones con estados **NORMAL** y **DEGRADADO**: el sistema detecta la pérdida del enlace (interferencias o pérdida de línea de visión) y actúa — en TELEOPERADO, **regresión automática** del recorrido hasta recuperar señal; en AUTÓNOMO, continúa la misión programada. |
...
| **RSW-02** | La documentación explicará la **arquitectura general** con diagramas **UML y/o SysML** más texto (módulos del vehículo, de los puestos de mando y de cualquier subsistema desarrollado), incluyendo el **detalle de las interfaces de entrada/salida** de cada módulo. |
| **RSW-04** | Son entregable SW: todos los **scripts, ficheros de configuración, datos de prueba, binarios y librerías** (online y offline), las **licencias** necesarias y un apartado de **dependencias** (librerías, compiladores/intérpretes, sistemas operativos, con sus versiones). |
...
| ID | Requisito (resumen) |
|---|---|
| **RDOC-05** | **Plan de Gestión del Proyecto**: objeto; descomposición estructurada en paquetes de trabajo, tareas y subtareas con entradas/salidas; organización y roles; programa de trabajos con **diagrama de Gantt** (≥ 3 niveles); referencias a Calidad, Riesgos y Configuración; perfil de esfuerzo. |
| **RDOC-09** | **Plan de Pruebas**: descripción de las pruebas de las Fases II y III; **matriz de trazabilidad pruebas ↔ requisitos**; recursos materiales y humanos; responsables por rol; planificación temporal; riesgos del plan. |
...
| **RDOC-17** | **Diseño de Software** — con **uso extensivo de diagramas SysML y/o UML**: estructura general con **diagrama de despliegue** (equipos que ejecutan software: puestos de operación, plataforma, cargas de pago, UAV de apoyo); interfaces entre módulos (lógicas y físicas); estructura de cada componente (diagramas de componentes); **casos de uso** con diagramas de secuencia y, si procede, de estados; dinámica de las GUI (pantallas y transiciones); herramientas de desarrollo y dependencias. |
